...
if __name__ == "__main__":
    ...
    # Stop execution here: the file contains an old appended solution.
    # Without this exit, Python reaches the old `if __name__ == "__main__"` block
    # and crashes (it references non-existent columns like `genres`).
    raise SystemExit(0)
import os
import sqlite3
from itertools import combinations
...
def parse_genres_from_artist_row(row: pd.Series):
    ...
    # Prefer explicit genre columns (more structured).
    genre_cols = [f"genre_{i}" for i in range(0, 7)]
    for c in genre_cols:
        if c in row.index:
            v = row.get(c, None)
            if pd.notna(v):
    ...
    # Fallback to artist_genres if present and genre_* columns are empty.
    if not genres and "artist_genres" in row.index:
        v = row.get("artist_genres", None)
        if pd.notna(v):
            s = str(v).strip()
...
def detect_and_clean_tracks_outliers():
    ...
    # 1) Remove invalid IDs and negative durations.
    tracks_clean = tracks.dropna(subset=["id"]).copy()
    if "track_popularity" in tracks_clean.columns:
        tracks_clean["track_popularity"] = pd.to_numeric(
            tracks_clean["track_popularity"], errors="coerce"
        )

    albums_clean = albums.dropna(subset=["track_id"]).copy()
    if "duration_ms" in albums_clean.columns:
        albums_clean["duration_ms"] = pd.to_numeric(
            albums_clean["duration_ms"], errors="coerce"
        )
        albums_clean = albums_clean[albums_clean["duration_ms"] > 0].copy()

    features_clean = features.dropna(subset=["id"]).copy()

    # Keep only rows that can be joined (helps avoid empty downstream joins).
    track_ids = set(tracks_clean["id"].dropna().unique())
    albums_clean = albums_clean[albums_clean["track_id"].isin(track_ids)].copy()
    feature_ids = set(features_clean["id"].dropna().unique())
    albums_clean = albums_clean[albums_clean["track_id"].isin(feature_ids)].copy()
    ...
    # 2) Detect outliers and address them:
    # - Cap popularity outliers using IQR
    if "track_popularity" in tracks_clean.columns and not tracks_clean["track_popularity"].dropna().empty:
        q1 = tracks_clean["track_popularity"].quantile(0.25)
        q3 = tracks_clean["track_popularity"].quantile(0.75)
        iqr = q3 - q1
        low = q1 - 1.5 * iqr
        high = q3 + 1.5 * iqr
        tracks_clean["track_popularity"] = tracks_clean["track_popularity"].clip(low, high)
...
def resolve_artist_duplicates_and_capitalization():
    ...
    # Numeric followers helps decide a canonical representative.
    artists["followers_num"] = pd.to_numeric(artists.get("followers", np.nan), errors="coerce")
    artists["followers_num"] = artists["followers_num"].fillna(-1)

    # Choose canonical id per normalized name (max followers, tie -> max id).
    idx = artists.groupby("name_low")["followers_num"].idxmax()
    canonical_ids = artists.loc[idx, ["id", "name_low"]].set_index("name_low")["id"].to_dict()
    artists["canonical_id"] = artists["name_low"].map(canonical_ids)
...
def add_era_to_albums_clean():
    ...
    albums_clean["era"] = albums_clean["release_date"].apply(categorize_era)
    albums_clean.to_sql("albums_data_clean_with_era", conn, if_exists="replace", index=False)
    conn.close()
...
def analyze_trends_over_time():
    ...
    """
    conn = connect_db()
    albums = pd.read_sql_query("SELECT track_id, release_date FROM albums_data_clean_with_era", conn)
    features = pd.read_sql_query("SELECT * FROM features_data_clean", conn)
    ...
    audio_cols = ["danceability", "energy", "loudness", "valence", "tempo"]
    audio_cols = [c for c in audio_cols if c in merged.columns]
...
def get_album_summary(album_name):
    """
    Part 4 bullet: write function that takes album name and outputs summary of album feature scores.
    """
    conn = connect_db()
    query = """
        SELECT f.danceability, f.energy, f.loudness, f.speechiness, f.acousticness, f.valence, f.tempo
        FROM albums_data_clean_with_era a
        JOIN features_data_clean f ON a.track_id = f.id
        WHERE lower(a.album_name) = lower(?)
    """
...
def plot_features_by_era(merged_df):
    """
    Part 4 bullet: group features by era (from albums data) and create barplots.
    """
    # merged_df includes release_date (from analyze_trends_over_time) but we need era.
    # So we re-join with albums_clean_with_era to ensure era comes from the dataset.
    conn = connect_db()
    era_lookup = pd.read_sql_query(
        "SELECT track_id, era FROM albums_data_clean_with_era",
        conn,
    )
    conn.close()

    df = merged_df.merge(era_lookup, left_on="track_id", right_on="track_id", how="left")
    df = df.dropna(subset=["era"])
    ...
    _ensure_plots_dir()
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS_DIR, "features_by_era.png"), dpi=150)
    plt.close(fig)
...
def plot_monthly_popularity_as_streams_proxy(top_n_tracks: int = 20):
    """
    Part 4 bullet: aggregate popularity and streams by month for songs with most total streams.
    Your DB schema has no 'streams' column; we use 'track_popularity' as proxy.
    """
    conn = connect_db()
    albums = pd.read_sql_query(
        "SELECT track_id, release_date FROM albums_data_clean_with_era",
        conn,
    )
    tracks = pd.read_sql_query(
        "SELECT id, track_popularity FROM tracks_data_clean",
        conn,
    )
    conn.close()
    ...
    # Total streams proxy per track = track_popularity (track appears once).
    track_totals = df.groupby("track_id")["track_popularity"].sum()
    top_track_ids = track_totals.nlargest(top_n_tracks).index.tolist()
...
def label_and_identify_genres(feature: str = "energy", q: int = 5):
    """
    Part 4 bullet: label tracks by feature intensity (very low..very high)
    and identify genres that occur frequently among very low and very high tracks.
    """
    conn = connect_db()
    # Join feature value with the track's primary artist genres.
    query = f"""
        SELECT f.{feature} AS feature_value,
               ar.genre_0, ar.genre_1, ar.genre_2, ar.genre_3, ar.genre_4, ar.genre_5, ar.genre_6,
               ar.artist_genres
    ...
    if df.empty or "feature_value" not in df.columns:
        print(f"No data available to label feature '{feature}'.")
        return
    ...
    # qcut can fail with too many duplicates; drop duplicates to keep it robust.
    df["intensity"] = pd.qcut(
        df["feature_value"],
        q=q,
        labels=labels,
...
if __name__ == "__main__":
    # Create cleaned tables (outliers + invalid records).
    detect_and_clean_tracks_outliers()
    resolve_artist_duplicates_and_capitalization()
    add_era_to_albums_clean()
    ...
    # Part 4 bullet: album feature summary.
    print("\nAlbum feature summary (example):")
    print(get_album_summary("Yours Truly"))
    ...
    # Part 4 bullet: aggregate popularity and streams by month.
    plot_monthly_popularity_as_streams_proxy(top_n_tracks=20)

    # Part 4 bullet: genre co-occurrence.
    analyze_genre_pairs()

    # Part 4 bullet: feature labeling.
    label_and_identify_genres("danceability")
import sqlite3
import pandas as pd
import numpy as np
...
# --- 1 & 2. DATA CLEANING: INVALID RECORDS & OUTLIERS ---
# Requirement: Remove invalid records (missing IDs, negative duration) [cite: 73]
# Requirement: Detect and address outliers in tracks/artists [cite: 72]
def wrangle_cleaning_and_outliers():
    conn = connect_db()
    tracks = pd.read_sql_query("SELECT * FROM tracks_data", conn)
    features = pd.read_sql_query("SELECT * FROM features_data", conn)
    
    # 1. Cleaning invalid records [cite: 73]
    initial_count = len(tracks)
    tracks = tracks.dropna(subset=['id'])
    # Filtering non-positive duration if column exists
    if 'duration_ms' in tracks.columns:
        tracks = tracks[tracks['duration_ms'] > 0]
    print(f"Cleaned {initial_count - len(tracks)} invalid records.")

    # 2. Outlier Detection using IQR [cite: 72]
    # Example for track_popularity
    Q1 = tracks['track_popularity'].quantile(0.25)
    Q3 = tracks['track_popularity'].quantile(0.75)
    IQR = Q3 - Q1
    lower_bound = Q1 - 1.5 * IQR
    upper_bound = Q3 + 1.5 * IQR
    
    outliers_count = len(tracks[(tracks['track_popularity'] < lower_bound) | 
                                (tracks['track_popularity'] > upper_bound)])
    print(f"Detected {outliers_count} outliers in track popularity.")
    
    conn.close()
    return tracks

# --- 3. TRENDS OVER TIME ---
# Requirement: Investigate the trend of features over time (averages by date) 
def analyze_trends_over_time():
    conn = connect_db()
    albums = pd.read_sql_query("SELECT track_id, release_date FROM albums_data", conn)
    features = pd.read_sql_query("SELECT * FROM features_data", conn)
    
    # Merging to get dates and features together [cite: 56, 74]
    merged = albums.merge(features, left_on='track_id', right_on='id')
    merged['release_date'] = pd.to_datetime(merged['release_date'], errors='coerce')
    
    audio_cols = ['danceability', 'energy', 'loudness', 'valence', 'tempo']
    daily_avg = merged.groupby('release_date')[audio_cols].mean().reset_index()
    
    print("\nSample of daily feature averages:")
    print(daily_avg.head())
    conn.close()
    return merged

# --- 4. ALBUM FEATURE SUMMARY ---
# Requirement: Function that takes an album name and outputs summary statistics 
def get_album_summary(album_name):
    conn = connect_db()
    query = """
        SELECT f.danceability, f.energy, f.loudness, f.speechiness, f.acousticness, f.valence
        FROM albums_data a
    ...
    print(f"\nSummary for album: {album_name} ")
    return df.describe().T[['mean', 'std', 'min', 'max']]

# --- 5. ARTIST DUPLICATES & CAPITALIZATION ---
# Requirement: Check artists_data for duplicates and capitalization issues 
def check_artist_duplicates():
    conn = connect_db()
    artists = pd.read_sql_query("SELECT id, name FROM artist_data", conn)
    
    # Normalize names to find capitalization issues 
    artists['name_low'] = artists['name'].str.lower().str.strip()
    name_counts = artists.groupby('name_low')['id'].nunique()
    duplicates = name_counts[name_counts > 1]
    
    print(f"\nFound {len(duplicates)} artist names associated with multiple IDs.")
    conn.close()

# --- 6. ERAS AND BARPLOTS ---
# Requirement: Group features by era and create barplots 
def plot_features_by_era(merged_df):
    def categorize_era(dt):
    ...
    merged_df['era'] = merged_df['release_date'].apply(categorize_era)
    era_avg = merged_df.groupby('era')[['danceability', 'energy', 'valence']].mean().reset_index()
    
    # Visualization 
    era_avg.set_index('era').plot(kind='bar', figsize=(10,6))
    plt.title("Average Audio Features by Era")
    plt.ylabel("Value")
    plt.xticks(rotation=45)
    plt.show()

# --- 7. MONTHLY AGGREGATION (POPULARITY AS STREAMS PROXY) ---
# Requirement: Aggregate popularity by month for top tracks 
def plot_monthly_popularity(merged_df):
    conn = connect_db()
    tracks = pd.read_sql_query("SELECT id, track_popularity FROM tracks_data", conn)
    
    # Join popularity
    df = merged_df.merge(tracks, left_on='track_id', right_on='id')
    top_100 = df.nlargest(100, 'track_popularity').copy()
    
    top_100['month'] = top_100['release_date'].dt.to_period('M')
    monthly = top_100.groupby('month')['track_popularity'].mean().reset_index()
    
    plt.figure(figsize=(12,4))
    sns.lineplot(data=monthly, x=monthly['month'].astype(str), y='track_popularity', marker='o')
    plt.title("Monthly Popularity Trend for Top 100 Tracks ")
    plt.xticks(rotation=90)
    plt.show()
    conn.close()

# --- 8. GENRE CO-OCCURRENCE ---
# Requirement: Identify genres that appear together most frequently 
def analyze_genre_pairs():
    conn = connect_db()
    artists = pd.read_sql_query("SELECT genres FROM artist_data", conn)
    
    genre_pairs = []
    for genres in artists['genres'].dropna():
        # Cleaning the string format from the DB 
        g_list = [g.strip().lower() for g in genres.replace("[", "").replace("]", "").replace("'", "").split(',')]
        if len(g_list) > 1:
            genre_pairs.extend(list(combinations(sorted(set(g_list)), 2)))
    
    print("\nMost Frequent Genre Pairs:")
    print(Counter(genre_pairs).most_common(5))
    conn.close()

# --- 9. FEATURE LABELING (VERY LOW TO VERY HIGH) ---
# Requirement: Label tracks by feature intensity and identify frequent genres 
def label_and_identify_genres(feature='energy'):
    conn = connect_db()
    # Joining features with artist genres [cite: 56, 81]
    query = f"""
        SELECT f.{feature}, a.genres
    ...
    # Binning into 5 categories 
    labels = ['very low', 'low', 'medium', 'high', 'very high']
    df['intensity'] = pd.qcut(df[feature], q=5, labels=labels)
    
    print(f"\nGenres frequently found in 'very high' {feature}:")
    high_genres = df[df['intensity'] == 'very high']['genres'].str.cat(sep=', ')
    # Simple count of words to identify frequent genres in that category
    print(pd.Series(high_genres.split(', ')).value_counts().head(5))
    conn.close()

# --- MAIN EXECUTION ---
if __name__ == "__main__":
    # Part 4 tasks execution
    cleaned_tracks = wrangle_cleaning_and_outliers() # Points 1 & 2
    full_data = analyze_trends_over_time()           # Point 3
    print(get_album_summary("Yours Truly"))         # Point 4
    check_artist_duplicates()                       # Point 5
    plot_features_by_era(full_data)                 # Point 6
    plot_monthly_popularity(full_data)              # Point 7
    analyze_genre_pairs()                           # Point 8
    label_and_identify_genres('danceability')       # Point 9
